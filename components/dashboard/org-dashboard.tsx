...
/** Signed-in member's org home — structural clone of the FG "MFD" daily briefing layout. */
export async function OrgDashboard({
  ...
  const stats: Stat[] = [
    { label: "Total Members", value: String(data.memberCount), icon: Users, color: "text-success" },
  ];
  ...
  return (
    <div className="p-3 sm:p-6 animate-page-enter">
      ...
      {/* Announcements — primary chassis (high priority) */}
      {data.news && data.news.latest.length > 0 && (
        <div className="mb-6">
          <MfdPanel
            ...
            titleAside={
              <a href="/news" className="flex items-center gap-1 mfd-label hover:text-primary">
                View All
                ...
                <a key={p.id} href={`/news/${p.id}`} className="block">
                  <div className="border border-primary/20 bg-primary/5 p-4 transition-colors hover:border-primary/40 hover:bg-primary/10">
                    <div className="mb-1 flex items-center gap-2">
                      {p.isPinned && <Badge variant="primary">PINNED</Badge>}
                      <CategoryBadge category={p.category} />
                      <span className="text-xs text-text-muted">
                        by <span className="text-text-secondary">{p.authorName}</span> · {formatDate(p.createdAt)}
                      </span>
                    </div>
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        {/* Activity Tape — neutral chassis, forums.recent as activity rows (lg:col-span-2) */}
              ...
              <ul className="space-y-2">
                {data.events.upcoming.map((e) => (
                  <li key={e.id}>
                    <a
                      href={`/events/${e.id}`}
                      className="flex items-center justify-between border border-border bg-surface-elevated px-3 py-2 transition-colors hover:border-primary/40 hover:bg-surface-hover"
                    >
                      <div className="min-w-0 flex-1">
                        <p className="truncate text-sm font-medium text-text-primary">{e.title}</p>
                        <p className="mfd-readout mt-0.5 text-[11px]">
