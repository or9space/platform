...
export function SquadCreateForm() {
  ...
  if (!open) return <button onClick={() => setOpen(true)} className="rounded bg-neutral-100 px-3 py-1.5 text-sm font-semibold text-neutral-900">New squad</button>;
  return (
...
export function AddMemberForm({ squadId }: { squadId: string }) {
  ...
  if (!open) return <button onClick={() => setOpen(true)} className="text-xs text-neutral-400 hover:text-neutral-200">+ Add member</button>;
  return (
    <form action={submit} className="mt-2 flex flex-wrap items-center gap-2">
      ...
      <input name="role" placeholder="role (optional)" maxLength={80} className="w-32 rounded border border-neutral-700 bg-neutral-900 px-2 py-1 text-xs" />
      <button type="submit" disabled={pending} className="rounded bg-neutral-100 px-2 py-1 text-xs font-semibold text-neutral-900 disabled:opacity-50">{pending ? "…" : "Add"}</button>
...
export function DeleteSquadButton({ id }: { id: string }) {
  const [pending, start] = useTransition();
  return <button onClick={() => start(async () => { const r = await deleteSquadAction(id); if (r.ok) window.location.reload(); })} disabled={pending} className="text-xs text-red-400 hover:text-red-300 disabled:opacity-50">{pending ? "…" : "Delete"}</button>;
}

export function RemoveMemberButton({ squadId, membershipId }: { squadId: string; membershipId: string }) {
  const [pending, start] = useTransition();
  return <button onClick={() => start(async () => { const r = await removeSquadMemberAction(squadId, membershipId); if (r.ok) window.location.reload(); })} disabled={pending} className="text-xs text-neutral-600 hover:text-red-400 disabled:opacity-50" title="Remove">×</button>;
}
