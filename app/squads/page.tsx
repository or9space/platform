...
export default async function SquadsPage() {
  ...
  return (
    <main className="mx-auto max-w-3xl space-y-6 p-6">
      ...
      {squads.length === 0 ? (
        <p className="text-sm text-text-muted">No squads yet.</p>
      ) : (
          ...
          {squads.map((s) => (
            <li key={s.id} className="rounded border border-border p-4">
              <div className="flex items-start justify-between gap-3">
                <div>
                  <p className="font-medium text-text-primary">{s.name} <span className="text-xs text-text-muted">· {s.members.length}</span></p>
              ...
              {s.members.length > 0 && (
                <ul className="mt-3 flex flex-wrap gap-2">
                  {s.members.map((m) => (
                    <li key={m.membershipId} className="flex items-center gap-1 rounded border border-border-light px-2 py-1 text-xs">
