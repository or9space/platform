      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            {post.isPinned && <span className="text-xs font-semibold uppercase text-amber">Pinned</span>}
            <CategoryBadge category={post.category} />
          </div>
          <h1 className="mt-2 text-2xl font-bold">{post.title}</h1>
          <p className="text-xs text-text-muted">{post.authorName} · {formatDateTime(post.createdAt)}</p>
        </div>
        {canManage && (
          <div className="flex shrink-0 items-center gap-2">
            <a href={`/news/${post.id}/edit`} className="rounded border border-border-light px-3 py-1.5 text-sm hover:border-primary">Edit</a>
            <DeleteNewsButton postId={post.id} />
          </div>
        )}
      </div>
      <article className="whitespace-pre-wrap text-text-primary">{post.body}</article>
    </main>
  );
}
