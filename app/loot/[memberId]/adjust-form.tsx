"use client";

import { useState, useTransition } from "react";
import { useRouter } from "next/navigation";
import { adjustLootAction } from "@/lib/actions/loot";

export function AdjustForm({ memberId }: { memberId: string }) {
  const [points, setPoints] = useState("");
  const [note, setNote] = useState("");
  const [error, setError] = useState<string | null>(null);
  const [pending, startTransition] = useTransition();
  const router = useRouter();

  function handleSubmit(formData: FormData) {
    if (pending) return;
    setError(null);

    const rawPoints = String(formData.get("points") ?? "").trim();
    const rawNote = String(formData.get("note") ?? "").trim();
    const parsed = parseFloat(rawPoints);

    if (!rawPoints || isNaN(parsed)) {
      setError("Enter a number of points (positive or negative).");
      return;
    }

    const amountTenths = Math.round(parsed * 10);
    if (amountTenths === 0) {
      setError("Adjustment cannot be zero.");
      return;
    }

    startTransition(async () => {
      const r = await adjustLootAction({
        memberId,
        amountTenths,
        note: rawNote || undefined,
      });
      if (!r.ok) {
        setError(r.error);
        return;
      }
      setPoints("");
      setNote("");
      router.refresh();
    });
  }

  return (
    <div className="rounded border border-border p-4">
      <h3 className="mb-3 font-semibold text-sm text-text-secondary">Adjust points</h3>
      <form action={handleSubmit} className="flex flex-wrap gap-3 items-end">
        {error && <p className="w-full text-sm text-fg-red-light">{error}</p>}
        <label className="flex flex-col gap-1 text-sm">
          Points
          <input
            name="points"
            type="number"
            step="0.1"
            value={points}
            onChange={(e) => setPoints(e.target.value)}
            disabled={pending}
            placeholder="+/- 0.0"
            className="w-28 rounded border border-border-light bg-surface px-3 py-2 disabled:opacity-50"
          />
        </label>
        <label className="flex flex-col gap-1 text-sm flex-1 min-w-40">
          Note (optional)
          <input
            name="note"
            type="text"
            value={note}
            onChange={(e) => setNote(e.target.value)}
            disabled={pending}
            placeholder="Reason…"
            maxLength={500}
            className="rounded border border-border-light bg-surface px-3 py-2 disabled:opacity-50"
          />
        </label>
        <button
          type="submit"
          disabled={pending}
          className="rounded bg-primary px-4 py-2 text-sm font-semibold text-fg-cream disabled:opacity-50"
        >
          {pending ? "Adjusting…" : "Adjust"}
        </button>
      </form>
    </div>
  );
}
