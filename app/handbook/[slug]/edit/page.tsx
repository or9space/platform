    ...
    <main className="mx-auto max-w-3xl p-6">
      <div className="mb-6 flex items-start justify-between gap-4">
        <div>
          ...
          {hb.subtitle && (
            <p className="mt-1 text-text-secondary">{hb.subtitle}</p>
          )}
          <p className="mt-1 text-xs text-text-muted">v{hb.version} — {hb.status}</p>
        </div>
        <a
          href={`/handbook/${slug}`}
          className="shrink-0 rounded border border-border-light px-3 py-1.5 text-sm hover:border-primary"
        >
          View
        </a>
      </div>
      ...
      <div className="mb-6">
        ...
        {hb.sections.length === 0 ? (
          <p className="mb-4 text-text-secondary">No sections yet.</p>
        ) : (
            ...
            {hb.sections.map((section) => (
              ...
              >
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-xs text-text-muted">Order: {section.orderIndex}</p>
                </div>
