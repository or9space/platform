      ...
      {eligibleTypes.map((t) => {
        ...
        return (
          <div key={t} className="rounded border border-border p-3">
            <p className="mb-2 font-mono text-sm text-text-secondary">{t} <span className="text-text-muted">({list.length}/3)</span></p>
            <ul className="mb-2 space-y-1">
              {list.map((d) => (
                <li key={d.key} className="flex items-center justify-between text-sm">
                  <span><code>{d.key}</code> · {d.label} · {d.kind}</span>
                  <button onClick={() => remove(t, d.key)} disabled={pending} className="text-xs text-fg-red-light hover:underline disabled:opacity-40">remove</button>
                </li>
              ))}
            </ul>
            {list.length < 3 && (
              <form action={(fd) => add(t, fd)} className="flex flex-wrap items-end gap-2">
                <input name="key" required placeholder="key" pattern="[a-z][a-z0-9_]{0,30}" className="w-28 rounded border border-border-light bg-surface p-1.5 text-sm" />
                <input name="label" required placeholder="Label" className="w-36 rounded border border-border-light bg-surface p-1.5 text-sm" />
                <select name="kind" className="rounded border border-border-light bg-surface p-1.5 text-sm">
                ...
                <button type="submit" disabled={pending} className="rounded bg-primary px-3 py-1.5 text-sm font-semibold text-fg-cream disabled:opacity-50">Add</button>
              </form>
            )}
          </div>
        );
