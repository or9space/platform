  ...
  return (
    <main className="mx-auto max-w-5xl space-y-8 p-6">
      <div className="flex items-center justify-between">
        ...
        {canManage && (
          <a href="/operations/new" className="rounded bg-primary px-3 py-1.5 text-sm font-semibold text-fg-cream">
            New operation
          </a>
        )}
      </div>
          ...
          {lanes.map((status) => (
            <section key={status}>
              <h2 className="mb-3 flex items-center gap-2 font-mono text-xs uppercase tracking-widest text-text-muted">
                {STATUS_LABELS[status]} <span className="text-text-muted">· {byStatus.get(status)!.length}</span>
              </h2>
